...
options:
  ...
  --run-dir=DIR          directory to create runtime files in [default: /var/run]
	`)
}
...
func runDaemon(args *docopt.Args) {
	...
	runDir := args.String["--run-dir"]
	if stat, err := os.Stat(runDir); os.IsNotExist(err) {
		...
		shutdown.Fatalf("--run-dir arg %q is not a directory", runDir)
	}
	...
	l, err := newHTTPListener(httpPort)
	if err != nil {
		shutdown.Fatal(err)
	}
	host.listener = l
