...
ln -s /usr/bin/netns-hack /usr/bin/sudo
...
echo "deb http://mirrors.syringanetworks.net/mariadb/repo/10.1/ubuntu xenial main" >> /etc/apt/sources.list.d/mariadb.list
...
# install packages
apt-get install -y postgresql-9.5 postgresql-contrib-9.5 redis-server mariadb-server-10.1 percona-xtrabackup mongodb-org
