...
func (s *MetricsSuite) TestSystemMetrics(t *c.C) {
	...
	// check "router_backend_http_status_total" matches router responses
	for i := 0; i < 3; i++ {
	...
	checkQuery(`sum(router_backend_http_status_total{app="status"}) by (status)`, model.Vector{
		{
			Metric: model.Metric{"status": "200"},
			Value:  3,
		},
	})
}
